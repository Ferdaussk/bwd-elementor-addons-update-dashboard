...
if($wp_query->have_posts()) :
    ...
    echo '<div class="bpiacrdn-single-img">';
        ...
        if($bpiacrdn_yes_value === $bpiacrdn_categories_swtcher){
            echo '<div class="bpiacrdn-prodCat '.esc_attr($settings['bpiacrdn_category_alignment']).'">';
            if('show_main_cat' === $bpiacrdn_cat_show_status){ 
                $categories = get_the_category();
                if ( ! empty( $categories ) ) {
                    echo '<ul class="post-categories"><li><a href="'.esc_url(plugin_dir_url('../../category/sk').$categories[0]->name).'" class="bpiacrdn-cat">'.esc_html( $categories[0]->name).'</a></li></ul>';	
                }
        ...
        echo ('yes' === !$bpiacrdn_image_link_show)?'<a class="bpiacrdn-for-overlay" href="'.esc_url(get_the_permalink()).'">':'<div class="bpiacrdn-for-overlay">';
            echo '<a class="bpiacrdn_thumbnail" href="'.esc_url(get_the_permalink()).'">'; $this->bpiacrdn_post_thumbnail_here();
            echo '<div class="bpiacrdn-content-box '.esc_attr($settings['bpiacrdn_content_vertical_alignment']).'">';
                $bpiacrdn_title_link_check = ('yes' === $bpiacrdn_title_link)?'<a href="'.esc_url(get_the_permalink()).'">'.esc_html(get_the_title()).'</a>':esc_html(get_the_title());
                $bpiacrdn_title_wrap = '<div class="bpiacrdn-title" style="text-align:'.esc_attr($settings['bpiacrdn_title_alignment']).'">'.$bpiacrdn_title_link_check.'</div>';
                echo ('yes' === $bpiacrdn_title)?wp_kses_post($bpiacrdn_title_wrap):'';
                echo '<div class="bpiacrdn-prodCont">';
                    echo '<div class="bpiacrdn-prodpriratbtn">';
                        echo '<div class="bpiacrdn-prodpriratwrap">';
                            // date
                            $bpiacrdn_post_date = get_the_date(esc_html($bpiacrdn_blog_date_format));
                            ...
                            echo wp_kses_post($bpiacrdn_date_switch.$bpiacrdn_date_title_switch);
                            // author
                            $bpiacrdn_control_icon = esc_attr($settings['bpiacrdn_author_icon']['value']);
                            $bpiacrdn_author_icon = '<i class="bpiacrdn_icons '.$bpiacrdn_control_icon.'"></i>';
                            $bpiacrdn_gravater = wp_kses_post(get_avatar( get_the_author_meta( 'ID' ), $size = '50', $default = '', $alt = 'Author Image', $args = array( 'class' => 'wt-author-img' ) ));
                            $bpiacrdn_total_author = ('icon' === $bpiacrdn_author_indicator) ? $bpiacrdn_author_icon : $bpiacrdn_gravater;
                            $bpiacrdn_author = ($bpiacrdn_yes_value === $bpiacrdn_author_swtcher) ? $bpiacrdn_total_author.'<span>'.get_the_author_meta( 'nickname' ).'</span>' : '';
                            echo '<div class="bpiacrdn-regularPrice bpiacrdn-price">'.wp_kses_post($bpiacrdn_author) .'</div>';
                        echo '</div>';
                    echo '</div>';
                    echo '<div class="bpiacrdn-prodOptInfo">';
                        ...
                        echo wp_kses_post($bpiacrdn_wrap_tag.$bpiacrdn_wrap_close_div);
                        // Comment
                        $bpiacrdn_main_txtComm = 'Comment ('.get_comments_number().')';
                        $bpiacrdn_main_comm = (!have_comments())?esc_html($bpiacrdn_main_txtComm):'';
                        ...
                        echo wp_kses_post($bpiacrdn_comm_switch.$bpiacrdn_comm_title_echo);
                        // =======================
                    echo '</div>';
                    // Description
                    ...
                    echo '<div class="bpiacrdn-prodDesc"><p class="bpiacrdn-decs">'.esc_html($bpiacrdn_description_switcher).'</p></div>';
                    // Button
                    $bpiacrdn_cart_text = '<a href="'.esc_url(get_the_permalink()).'" class="bpiacrdn-cartBtn">'.esc_html($bpiacrdn_cart_button).'</a>';
                    $bpiacrdn_cart_btn_icon = ($settings['bpiacrdn_cart_type'] == 'icon')?$settings['bpiacrdn_cart_button_icon']['value']:'';
                    $bpiacrdn_cart_icon = '<a href="'.esc_url(get_the_permalink()).'" class="bpiacrdn-sscartbtndd"><i class="bpiacrdn-cartBtn '.esc_attr($bpiacrdn_cart_btn_icon).'"></i></a>';
                    ...
                    echo ('yes' === $bpiacrdn_cart_btn)?wp_kses_post($bpiacrdn_cart_wrap_html):'';
                echo '</div>';
            echo '</div>';
        echo ('yes' === !$bpiacrdn_image_link_show)?'</a>':'</div>';
    echo '</div>';
endwhile;
echo '</div>';
    ...
    if ($total_pages > 1){
        ...
        if('none' === $bpiacrdn_the_pagination_type){
            ...
            echo wp_kses_post(paginate_links(array(
                ...
                'total' => $total_pages,
                'prev_text' => esc_html($settings['bpiacrdn_blog_prev_format']),
                'next_text' => esc_html($settings['bpiacrdn_blog_next_format']),
            )));
