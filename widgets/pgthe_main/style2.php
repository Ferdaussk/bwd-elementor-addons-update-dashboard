...
?>
<div class="crpt-card-area-2">
    <div class="container-fluid">
        <div class="row <?php echo esc_attr($crtp_align); ?> crtp_gap_r">
        <?php
        $crtp_all_products = new WP_Query( $crtp_args ); if($crtp_all_products->have_posts()):
        while ( $crtp_all_products->have_posts() ) : $crtp_all_products->the_post();
            ...
            <div class="col-lg-4 col-md-4 col-sm-6 column_number crtp_gap_c">
                <div class="crpt-card">
					...
					<?php } else{echo '<div class="crpt-img_text">'.esc_html__('No Feature Image', 'bwd-elementor-addons').'</div>';} ?>
                    <?php if('yes' === $settings['crtp_cart_btn']){ ?>
                    <div class="crpt-card-icon">
											<?php echo '<a href="' . esc_url($product->add_to_cart_url()) . '"><i class="'.esc_attr($settings['crtp_cart_button_icon']['value']).'"></i></a>'; ?>
                    </div>
                    <?php } ?>
                    <div class="crpt-contentbx">
						<?php if('yes' === $settings['crtp_title']){ ?>
                        <div class="crpt-card-heading <?php echo esc_attr($crpt_cont_align); ?>"><a href="<?php echo esc_url(get_permalink()); ?>"><?php echo esc_html(get_the_title()); ?></a></div>
						<?php } ?>
                       ...
                       <div class="crpt-category-tag <?php echo esc_attr($crpt_cont_align); ?>">
                        ...
                        <div class="crpt-btn-ptype <?php echo esc_attr($crpt_cont_align); ?>">
							<?php if('yes' === $settings['crtp_btn']){ ?>
                            <div class="crpt-card-button">
                                <a href="<?php if('default' === $crtp_choose_link){ echo esc_url(get_permalink()); } elseif('coustom_link' === $crtp_choose_link){ echo esc_url($settings['crtp_main_button_link']['url']); } ?>"><?php echo esc_html($crtp_main_button); ?></a>
                            </div>
							<?php } ?>
